...
void TrackerSystematicMisalignments::analyze(const edm::Event& event, const edm::EventSetup& setup){
	...
	//The surface deformations are applied to the TrackerGeometry, so they need to be done
	//  before creating the AlignableTracker from it
		...
		if (oldDeformation) {
			...
			switch (type) {
				case SurfaceDeformationFactory::kBowedSurface:
					for (int i = 0; i < 3; i++)
						newparams.push_back(addparams[i]);
				case SurfaceDeformationFactory::kTwoBowedSurfaces:
					for (int i = 0; i < 12; i++)
						newparams.push_back(addparams[i]);
					TwoBowedSurfacesDeformation *oldTwoBowedDeformation = dynamic_cast<const TwoBowedSurfacesDeformation*>(oldDeformation);
					if (!oldTwoBowedDeformation)
						throw cms::Exception("GeometryError")
						<< "[TrackerSystematicMisalignments] " << geomDetUnit->geographicalId()
						<< " has the wrong kind of deformations!";
					newparams.push_back(oldDeformation->parameters()[oldTwoBowedDeformation->k_ySplit()]);
			}
			SurfaceDeformation *newDeformation = SurfaceDeformationFactory::create(type, newparams);
			oldDeformation->add(*newDeformation);
		}
		else
			edm::LogWarning("MisalignedTracker")
			<< "GeomDetUnit " << geomDetUnit->geographicalId() << " has no surface deformations!  Can't add to it.";
}
